            ...
            <div class="space-y-2 py-6">
              @foreach($navMenuItems['items'] as $navMenuItem)
                <a href="{{ $navMenuItem['url'] }}"
                   ...
                   class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold hover:bg-gray-50 !no-underline hover:text-primary {{ $navMenuItem['is_current'] ? 'text-primary' : 'text-gray-900' }}">
                  {{ $navMenuItem['label'] }}
                </a>
              @endforeach
            </div>
